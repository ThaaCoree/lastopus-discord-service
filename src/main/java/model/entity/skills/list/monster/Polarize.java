...
public class Polarize extends Skill {

    public static String NAME = "Polarize";

    public Polarize() {
        super();
        setDescription("สั่งใช้งานได้ทันทีที่องค์ประกอบของโพลาร์ไลซ์เสร็จสิ้น\n" +
                "ทำให้พลังชีวิตของยูนิตศัตรูที่ได้รับผลทั้งหมดเหลือ 0.1 หน่วย, หยุดแอคชันที่ยูนิตเป้าหมายทั้งหมดที่กำลังกระทำและทำให้จบเทิร์น\n" +
                "ในรอบเทิร์นนี้ เทิร์นของยูนิตศัตรูทั้งหมดที่ได้รับผลจะถูกข้าม");
        setActionType("-");
        setManaCost(0);
        setCooldown(0);
    ...
    public void calculateBehavior(CombatFlow combatFlow, SkillTarget skillTarget) {
        if (!skillTarget.getTarget(0).isEmpty()) {
            ...
            for (String name : skillTarget.getTarget(0)) {
                ...
                if (unit.getHealth().getRemaining() > 0) {
                    unit.setRemainingHealth(0.1);
                }
            }
        }
    }
