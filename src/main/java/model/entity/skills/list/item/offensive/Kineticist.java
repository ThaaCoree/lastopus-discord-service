...
public class Kineticist extends Skill {

    public static String NAME = "Kineticist";

    public Kineticist() {
        super();
        setDescription("ความเสียหายเวทมนตร์ทั้งหมดจะกลายเป็นกายภาพ");
            ...
            for (Unit unit : event.unit_target) {
                for (ActionEffect actionEffect : event.effects.get(unit.getName())) {
                    if (actionEffect.type == ActionEffectType.DAMAGE_MAGICAL) {
                        ...
                        LogWriterUtil.log(">Kineticist triggered");
                    }
                }
            }
