...
public class Seal_Of_Hope extends Skill {

    public static String NAME = "Seal of Hope";

    public Seal_Of_Hope() {
        super();
        setDescription("ในระหว่างที่ถูก Imprison การฮีลทั้งหมดเพิ่มขึ้นเป็นสองเท่า");
...
//                , 0, 1);
    ...
    public void initializeEvent(CombatFlow combatFlow) {
        ...
        eventBus.register(ActionEvent.class, EventPhase.MODIFY, -1, (ActionEvent event) -> {
            if (event.unit_source != getUser() || !getUser().hasCondition("Imprison") || !event.hasActType(ActType.HEAL)) return;
            for (Unit unit : event.unit_target) {
                if (!event.canHeal(unit.getName())) continue;
                for (ActionEffect actionEffect : event.effects.get(unit.getName())) {
                    if (actionEffect.type == ActionEffectType.HEALTH_RECOVER) {
                        actionEffect.finalValue *= 2;
                    }
                }
            }
        });
