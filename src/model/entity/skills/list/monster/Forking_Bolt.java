...
public class Forking_Bolt extends Skill {

    public static String NAME = "Forking Bolt";

    public Forking_Bolt() {
        super();
        setDescription("ยิงกระสุนมนตราที่สร้างความเสียหายเวท XA หน่วย เมื่อตกกระทบ แยกออกเป็น XB ลูกไปด้านหลัง");
...
//                , 0, 1);
    ...
    public void calculateBehavior(CombatFlow combatFlow, SkillTarget skillTarget) {
        if (!skillTarget.getTarget(0).isEmpty()) {
            ...
            sendActionEvent(combatFlow.getEventBus(),
                    ActionEvent.builder(getName(), getUser(), combatFlow.findUnit(skillTarget.getTarget(0)))
                            .effect(ActionEffectType.DAMAGE_MAGICAL, xa, 2)
                            .addActType(ActType.CAST, ActType.STRIKE)
                            .build()
            );
        }
